...
from data_utils.data_collator import SelectiveDPDataCollator
...
def compute_ppl_for_text(model, text: str, tokenizer) -> float:
    """
    计算给定 text 的 perplexity。
    """
    model.eval()
    with torch.no_grad():
        input_ids = tokenizer.encode(text, return_tensors="pt").to(model.device)
        # labels = same as input_ids, 计算语言模型 PPL
        outputs = model(input_ids, labels=input_ids)
        # outputs.loss 是平均到各个 token 的 CrossEntropy
        loss = outputs.loss.item()
        ppl = math.exp(loss)
    return ppl


def random_subset_of_train_dataset(dataset, size=1000, seed=42):
...
def evaluate_model_on_sensitive_tokens(
        model,
        dataset,
        batch_size=8,
        device="cuda",
        ...
        """
        在给定的数据集上评估模型对每个 token_id 的平均预测概率。
        不使用自定义的 SelectiveDPDataCollator，而是使用一个默认的 collator，
        这样就不会丢弃 < 和 >。
        """
        ...
        # 2) 准备 DataLoader
        #    注意这里使用默认 collator => 保留所有 token，不做额外处理
        loader = DataLoader(
            dataset,
        ...
        token_counts = {}       # {token_id: count_of_occurrences}
        token_probs_sum = {}    # {token_id: sum_of_model_pred_probs_for_correct_label}

        # 3) 遍历数据
        for batch in loader:
            # 把 batch 的张量放到同一个 device
            ...
            with torch.no_grad():
                ...
                # shift logits 和 labels，跟语言模型训练时对齐
                ...
                # 获取“正确标签”的概率
                # shift_labels.unsqueeze(-1) => (B, seq_len-1, 1)
                correct_label_probs = probs.gather(
                    dim=-1, index=shift_labels.unsqueeze(-1)
                ).squeeze(-1)  # => (B, seq_len-1)

            # 4) 遍历每个位置，累加统计
            B, L = correct_label_probs.shape
            for i in range(B):
                for j in range(L):
                    lbl = shift_labels[i, j].item()
                    if lbl == -100:
                        # 忽略 padding 或不计分位置
                        continue
                    p = correct_label_probs[i, j].item()

                    token_counts[lbl] = token_counts.get(lbl, 0) + 1
                    token_probs_sum[lbl] = token_probs_sum.get(lbl, 0.0) + p

        # 5) 计算平均预测概率
        avg_prob_for_token_id = {}
        for tid, c in token_counts.items():
            avg_prob_for_token_id[tid] = token_probs_sum[tid] / c

        return avg_prob_for_token_id
...
class Trainer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_params = sum(param.numel() for param in self.model.parameters() if param.requires_grad)
        ...
        self.eval_data_collator = eval_data_collator  # <<< 新增
        self.train_dataset = train_dataset
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        ...
        collator_for_eval = self.eval_data_collator if self.eval_data_collator is not None else self.data_collator
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        # 如果 用户已经 显式提供了 optimizer & lr_scheduler, 直接跳过
        if self.optimizer is not None and self.lr_scheduler is not None:
            return

        # 如果 self.optimizer还是None，就创建默认AdamW
        ...
        # 如果 self.lr_scheduler还是None，就创建默认 linear_schedule
    ...
    def train(self, model_path: Optional[str] = None, **kwargs):
        print(">>> DEBUG: We are inside my custom train function!")
        ...
        print(">>> DEBUG: We are inside my custom loop function!")

        
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            self.data_collator.set_current_epoch(epoch + 1)  # 或者 epoch, 看你的需要
            print(f"[DEBUG] Start of epoch loop: epoch={epoch} (range={epochs_trained}~{int(np.ceil(num_train_epochs))})")
            ...
            # 比如只在每 5 个 epoch 后更新:
            #if (epoch + 1) % 5 == 0:
             #   logger.info(f"Dynamic update of loss_weights at epoch {epoch+1}")
               # self.update_loss_weights(model)    
            print(f"[DEBUG] End of epoch {epoch} loop, about to do dynamic update check...")

            # 在 epoch 循环末尾、即将结束时，插入
            print(f"[DEBUG] End of epoch {epoch}, about to compute canary PPL")

            canary_text = "My ID is 341752."
            canary_ppl = compute_ppl_for_text(model, canary_text, self.tokenizer)
            print(f"[DEBUG] Canary PPL = {canary_ppl}")

            

            # 若想做更多记录（比如 TB日志），可以：
            # self.tb_writer.add_scalar("canary/ppl", canary_ppl, self.global_step)

   
            # 1) 只在第 2,3,4 个 epoch (也就是 1<(epoch+1)<5) 修改噪声
            if 0 <= (epoch + 1) < 6:
                if not self.privacy_args.non_private:
                    dp_optimizer = self.optimizer

                    # 改用 privacy_engine.noise_multiplier
                    dp_engine = dp_optimizer.privacy_engine

                    old_noise = dp_engine.noise_multiplier
                    new_noise = old_noise * 1.2
                    dp_engine.noise_multiplier = new_noise
                    print(f"[DP] epoch={epoch+1}, noise changed: {old_noise} -> {new_noise}")


                print(f"[DP] epoch={epoch+1}, noise changed: {old_noise} -> {new_noise}")
            # 2) 只在每 5 个 epoch 时做 subset/eval/stats 更新
            if (epoch + 1) % 5 == 0:
                subset = random_subset_of_train_dataset(self.train_dataset, size=10000)
                stats = evaluate_model_on_sensitive_tokens(model, subset)
                self.data_collator.update_sensitive_stats(stats)
            ...
            if (
                self.args.evaluation_strategy in (EvaluationStrategy.EPOCH, IntervalStrategy.EPOCH)
    ...
    def compute_loss(self, model, inputs):
        ...
        # 1) 取出 labels, loss_weights（若没有则为None）
        labels = inputs.pop("labels")
        loss_weights = inputs.pop("loss_weights", None)  # (B, seq_len) or None

        # 2) 前向计算
        outputs = model(**inputs)
        logits = outputs.logits  # shape (B, seq_len, vocab_size)

        # 3) 做 next-token prediction shift
        #    shift_logits => (B, seq_len-1, vocab_size)
        #    shift_labels => (B, seq_len-1)
        shift_logits = logits[..., :-1, :].contiguous()
        shift_labels = labels[..., 1:].contiguous()

        # 若有 loss_weights，也做相同 shift
        if loss_weights is not None:
            shift_weights = loss_weights[..., 1:].contiguous()  # (B, seq_len-1)
        else:
            shift_weights = None

        # 4) 计算序列中有效token数量(非-100处)
        seq_lens = (shift_labels != self.ignore_index).sum(dim=1).clamp(min=1)  # 防止除0

        # 5) 使用 CrossEntropy: reduction="none" 得到 (B, seq_len-1)
        per_token_loss = torch.nn.functional.cross_entropy(
            shift_logits.permute(0, 2, 1),  # => (B, vocab_size, seq_len-1)
            shift_labels,                   # => (B, seq_len-1)
            reduction="none",
            ignore_index=self.ignore_index,
        )

        # 6) 若存在自定义的 loss_weights, 对 loss 做元素乘
        if shift_weights is not None:
            per_token_loss = per_token_loss * shift_weights

        # 7) 在 seq_len-1 维度上求和，并除以有效token数 => (B,)
        #    这样返回的是“每个样本”的loss
        loss = per_token_loss.sum(dim=1) / seq_lens

        # 8) 如果你还需要保存 past或做别的事情，保持不变
        if self.args.past_index >= 0:
            self._past = outputs[self.args.past_index]

        # 9) 返回形状 (batch_size,) 的 1D张量
        return loss
