...
class FakeLenWikiDataset(Dataset):
    """
    no_brackets: chunked tokens => 用于 __len__()
    with_brackets: chunked tokens => 用于 __getitem__()
    这样 Trainer iteration 就会基于 no_brackets数量，
    实际 Collator 拿到 with_brackets tokens (含 <...>).
    """
    def __init__(self, no_brackets: dict, with_brackets: dict):
        self.nb = no_brackets
        self.wb = with_brackets

        nb_len = len(self.nb["input_ids"])
        wb_len = len(self.wb["input_ids"])
        self.common_len = min(nb_len, wb_len)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for k in self.wb.keys():
            ex[k] = torch.tensor(self.wb[k][i], dtype=torch.long)
        return ex
...
class BlockByBlockWikiText2TextDataset(Dataset):
    def __init__(
        ...
        self.seed = 1111
        self.normalized_canary = "My ID is <341752>."
        ...
        # 1) tokenize + group_texts for train no_brackets / with_brackets
        tokenized_train_nb = tokenizer(raw_datasets["train_no_brackets"])
        tokenized_train_wb = tokenizer(raw_datasets["train_with_brackets"])

        grouped_train_nb = group_texts(tokenized_train_nb)
        grouped_train_wb = group_texts(tokenized_train_wb)

        # 构造 FakeLenWikiDataset => iteration基于no_brackets
        self.train_examples = FakeLenWikiDataset(
            no_brackets=grouped_train_nb,
            with_brackets=grouped_train_wb
        )

        # 2) valid/test => 只做单份
        tokenized_valid = tokenizer(raw_datasets["valid_original"])
        tokenized_test  = tokenizer(raw_datasets["test_original"])
        grouped_valid = group_texts(tokenized_valid)
        grouped_test  = group_texts(tokenized_test)

        # 用WikiDataset包装
        self.val_examples = WikiDataset(grouped_valid)
        self.test_examples= WikiDataset(grouped_test)
    ...
    def _load_raw_datasets(
        ...
        # 2) 如果你要插入 canary，就和原逻辑一样对 train_lines 做插入
        # pdb.set_trace()
        if add_canary:
        ...
        # ============ 生成 train_no_brackets, train_with_brackets =============
        train_no_brackets   = [remove_brackets(ln) for ln in train_lines]
        train_with_brackets = train_lines  # 保留原行

        # valid, test 保持原逻辑(无 `<...>` 无需两份)
        valid_join = "".join(valid_lines)
        test_join  = "".join(test_lines)

        raw_datasets = {
            # train
            "train_no_brackets"  : "".join(train_no_brackets),
            "train_with_brackets": "".join(train_with_brackets),

            # valid, test
            "valid_original": valid_join,
            "test_original" : test_join,
        }
